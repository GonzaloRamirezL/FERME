$(document).ready(function() {  
    ...
    ventaBuscar = new IndexVentaBuscar();
    cerrarModalVenta();
});

function cerrarModalVenta() {

    $("#btnCerrarModalVenta").click(function () {
        $('#ModalDetalleVentaCliente').modal('toggle');
    })
}

function IndexVentaBuscar() {
...
function anularVentaCliente(stDatos) {
    swal({
        title: "¿Está seguro?",
    ...
    if (willDelete) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                destruirTablaVentas();
                $('#ModalDetalleVentaCliente').modal('hide');
            },
            error: function () {
                console.log("no funciona")
            }
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index, value) {
                        ...
                        fila += '<td>' + (value[2] )+ '</td>';
                        fila += '<td>' + (value[3] )+ '</td>';
                        fila+= "<td><a class='btn btn-success' href='javascript:ventaBuscar.editar("+value[0]+","+value[7]+")'>Detalles</a></td>";
